...
传统的对象存储用一个能力来授权对一个对象的访问。因此，对N个对象的预取会产生N个能力。由于能力的产生是需要消耗计算资源的，因此，当预取的信息最后没有使用，这就会造成很大的开销。

我们扩展了这个模型以便能够让一个能力可以授权对多个对象的访问。多对象能够减少预取开销，还能够减少总共生成的能力数目，从而减少总的元数据服务器CPU负载。然而，能力的大小还是很小-回忆前面所讲的，客户端将能力作为请求的一部分发送给对象存储设备。理想情况下，无论传送多少个对象，能力是很紧凑的而且大小固定。

这种理想情况可以通过下面的方式实现：能力包含一个范围中的对象ID，这些对象ID的映射信息是固定的，而且它们的访问权限也是一样的。一个范围可以通过开始和结束的对象ID定义。一系列相关的对象的映射信息通常是相似的，可以通过一个存储设备列表指定，并且可以通过一个简单的策略决定位置。总之，这样一个能力的大小通常是单个对象能力大小的两倍(少于100字节)。

当采用名字空间平台化时，范围是很自然的选择，因为连续的对象ID在目录结构中有局部性。如果用户可以访问一个目录中的某个文件，他通常也有权限访问该目录和相邻目录的其它文件。(Note that the permissions in question are for the client machine)，因此，许多文件权限的细微差别并不影响对多个对象产生单个能力。

即便如此，当指定一个范围的对象时，还是要小心定义访问权限。具体地，访问权限必须依赖于存储设备来帮助决定是否有对应的权限。对于读来说，访问权限应该是“如果对象存在就允许读”，以便允许很稀疏的范围。对于重写来说，访问权限应该是“允许在已经存在的对象中存在的字节”，以便允许写而空间不会无限增长。当必须控制空间使用时，如果需要创建新的数据，就要单独与元数据服务器交互。

实现多对象能力要做的一个额外改变是更新信息。在OSD的规范中，存储设备保存每个对象的版本号。当操作可能限制以前对对象的能力(例如，删除对象或者改变访问权限)时，元数据服务器更新这个版本号。当客户端请求能力，元数据服务器会在能力中包含版本号。当客户端提供的版本号与存储设备中保存的对象的版本号一致，存储设备才会验证这个能力是有效的。为了防止在一组能力中要求每个对象的分离的版本号，我们做了如下改变。我们要求元数据服务器在更新存储设备的版本号和发送版本号时使用单调递增的版本号。我们要求存储设备验证一个版本号大于存储的版本号，而不是等于存储的版本号。做了这两个改变，一组能力可以包含一个版本号，在能力被发送出去之前，这个版本号对所有更新的对象都是有效的。除了能够使大小一致，这么做还能使能力对一组对象中的某些对象有效，(even if it is no longer current for other objects)。

## 3.2 名字空间平坦化

名字空间平坦化是一种对象ID分配策略，这种策略将目录结构编码进对象ID空间。两种常见的分配策略是伪随机和创建顺序。伪随机策略通过一个随机数生成器或者计算文件名或内容的哈希分配对象ID；而不保存任何位置信息。创建顺序策略保存一个计数器，将下一个值分配给新创建的对象。当创建顺序跟访问顺序一致，这就保存了位置信息，但是经过一段时间之后，就会产生很多碎片。这部分详细解释碎片问题，描述一些名字空间平坦化算法。

### 3.2.1 对象ID空间的碎片

在传统的基于块的文件系统中，对象ID的分配算法受到类似的索引节点碎片问题。由于对象ID是对象创建时分配的，之后的某些操作会改变名字空间，这就会扰乱类似创建顺序的策略带来的名字空间位置和对象ID相邻的关系。随着时间的推移，这种干扰会慢慢随机化这种关系，减少位置关系：名字空间是一个很好的预测。(回忆下，所有的现代文件系统都用这种方式依赖于名字空间)

简单的算法，比如创建顺序，会受到碎片相关问题的影响，因为，它创建的是完全的密集的对象ID空间，没有在目录中留下任何增长或者减少的空间。比如，不在同一天的两个文件在同一个目录中创建，按照创建顺序，它们就有可能分配完全不同的对象ID(相差很远)。图2表示了创建顺序策略易受到碎片的影响。这些图展示了从CVS仓库导出的linux源代码树中用find明亮查找一个不存在的文件时，根据创建顺序和遍历顺序分配对象ID的对比。最左边的图展示了从仓库中导出的linux源代码库的初始状态。cvs checkout命令以深度优先顺序创建文件，因此，对象ID与用find命令遍历时的顺序一致。这样，对于深度优先搜索，可以预取正确的对象。中间这个图展示了从CVS仓库获得15个补丁并将补丁添加到源码树中之后的状态。导致的碎片结果很明显，相邻时间访问的条目的对象ID已经不连续了。打31个补丁后的结果更明显。

![](https://github.com/luofengmacheng/translation/raw/master/pic/pic7.png)

图2：经过时间的推移，使用创建顺序分配算法时对象ID空间的碎片

另外一个会导致碎片的是RENAME操作，它会将一些文件移动到另一个目录中，这就会导致原来的分配选择和新的名字空间位置不一致。幸运的是，这样的操作在实际中很少，并不多-5.2部分中对trace的分析表明它们在所有操作中的比例小于0.001%。

系统可以主动地或者被动地处理碎片。大多数文件系统会主动地对ID空间进行分段，从而将不相关的增长和减少独立。相关的对象在ID空间的同一个段中，在某个段中的分配和回收并不影响其它段。一些文件系统被动地进行碎片整理，它们通过周期性的扫描，然后改变ID的分配来达到理想情况。一些碎片整理方案在大规模分布式存储系统中很难实现，但是，它们可以对主动方案进行补充。
...
<tr>
	...
	<td>/home/</td>
	<td>/home/person/</td>
	<td>/home/person/submit.ps</td>
	<td>/home/person/submit.pdf</td>
</tr>
<tr>
	<td>Child-closest</td>
	<td>0x0400 0000</td>
...
表1：名字平坦化的例子。这个表展示了在一个目录层次中使用两种名字空间平坦化策略时，对一些文件可能使用的对象ID。所有的数字都是十六进制。

### 3.2.2 名字空间平坦化算法

这部分描述了两个用于分配对象ID和主动防止碎片的名字空间分配算法。对对象ID进行分段，每个目录层次对应一个槽(slot)，将每个目录的内容放到它自己的槽中，从而防止碎片。首先，child-closest静态分配深度，显示地将相邻的子树放到OID空间中。其次，cousin-closest用移位将同一层次相邻的目录放到OID空间中。表1对这两种情况进行了解释，显示了文件名可能被如何转换成OID。表1和这部分的所有例子都以16进制显示32位的OID。

**Child-Closest：**child-closest算法给每个目录层次分配一个槽，从左到右移动，并且给每个目录中的文件或者目录分配一个数字。比如，如果/home/分配的是0x4a，它的子目录person/分配的是0xa，那么/home/person/submit.pdf的ID会以0x4a开始，这里假设每个槽被分配为4位。文件数目的增长是从左向右的。如果submit.pdf分配的是0x8，那么/home/person/submit.pdf的ID就是0x04a00008。包含目录内容的对象通常是这个目录中的零值文件，于是，在这个目录空间中，分配的数字是0x0。

child-closest算法在深度优先遍历和相似访问模式的负载的性能很好。特别地，用这个算法，目录和文件的相邻性可以用它们在目录层次中的垂直距离衡量；一个目录的相邻目录的OID是相邻的。这种策略的一个额外特性是可以用一个范围表示整个子目录。例如，0xae410000--0xae41FFFF可能表示/home/person/cvs/project/src整个子目录。

**Cousin-Closest**：cousin-closest算法也使用槽，给目录中的每个文件和子目录分配一个数字。但是，它的对象ID是从右到左增长的，父目录跨越了槽。例如，如果/home/分配的是0x4，person/分配的是0xa，submit.pdf分配的是0x8，那么，/home/person/submit.pdf就是0x000004a8。跟child-closest一样，包含目录的对象是这个目录的零值文件。

cousin-closest算法对于广度优先搜索和类似访问模式的负载的性能很好。使用这个算法，目录和文件的对象ID的相邻性是它们在目录层次中的水平距离的函数。cousin-closest的一个额外特性是它更适合产生目录层次密集的表示。
